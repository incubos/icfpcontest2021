...
public class SimulationBody extends Body {
	 ...
	 * Default constructor.
	 */
	public SimulationBody(String name, Color color) {
		this.name=name;
		this.color = color;
	}
	public SimulationBody(String name) {
		this(name,Graphics2DRenderer.getRandomColor());
	}
	...
	public void render(Graphics2D g, double scale, Color color) {
		...
		lt.transform(new Point2D.Double(ce.x,ce.y), aDouble);
		g.setColor(Color.WHITE);
		g.fill(ce);
		g.setColor(Color.DARK_GRAY);
		g.draw(ce);
//		System.out.println(name+" tr : "+this.getTransform().getTranslation());
